def game_snake():
    ...
    dirs=2
    loading=load('Snake\Menu\\Snake Menu.png')
    dot=load('Snake\Menu\\loadDot.png')
    def collide(x1, x2, y1, y2, w1, w2, h1, h2):
        if x1+w1>x2 and x1<x2+w2 and y1+h1>y2 and y1<y2+h2:return True
        else:return False
    f = pygame.font.SysFont('Arial', 30);
    def die(sc):
        fi=open('Snake\highscores.txt','a+')
        highsc=fi.read()
        
        while(True):
            

            t=f.render('GAME OVER : YOUR SCORE WAS : %d' % sc, True, (blue));
            screen.blit(t, (30,140));
            
            if sc<int(highsc):
                t1=f.render('THE HIGH SCORE IS : %s' % highsc, True, (blue))
                screen.blit(t1,(60,250))
                pygame.display.update()
                fi.close()
                
            else:
                t1=f.render('CONGRATULATIONS!!!YOU BEAT THE HIGH SCORE',True,(blue))
                screen.blit(t1,(10,250))
                highsc=str(sc)
                pygame.display.update()
                fi.close()
                os.remove('Snake\highscores.txt')

                fil=open('Snake\temp.txt','w')
                
                fil.write(highsc)
                fil.close()
                os.rename('Snake\temp.txt','Snake\highscores.txt')
